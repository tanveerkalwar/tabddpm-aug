...
def compute_dcr_statistics(real_data, synthetic_data):
    """
    Compute Distance-to-Closest-Real (DCR) statistics using vectorized euclidean distances.
    ...
    dist_matrix = euclidean_distances(synthetic_data, real_data)
    dcr_vals = dist_matrix.min(axis=1)
    return {
        'mean_dcr': float(np.mean(dcr_vals)),
        'median_dcr': float(np.median(dcr_vals)),
        'min_dcr': float(np.min(dcr_vals)),
        'pct_above_4': float(np.mean(dcr_vals > 4.0) * 100)
    }
